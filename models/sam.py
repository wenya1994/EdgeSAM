...
# Adaptive Progressive Learning Loss
def APLloss(pred, feat, mask=False, epoch=1, config=None, name=None):
...
def Loss(pre1, pre2, epoch, ws, config, name):
    sal1 = pre1['sal'][0]
    sal2 = pre2['sal'][0]
    ws = [0.4, 0.3, 0.3]
    ...
    apl_loss = APLloss(p1, pre1['feat'][0], False, epoch, config, name) + APLloss(p2, pre2['feat'][0], False, epoch,
                                                                                  config, name)
    bceLoss = BBCEWithLogitLoss()
    p2 = F.interpolate(p2, size=p1.size()[2:], mode='bilinear', align_corners=True)
    mse_loss = torch.mean(torch.pow(p1 - p2, 2))

    return bceLoss * ws[0], apl_loss * ws[1], mse_loss * ws[2]
